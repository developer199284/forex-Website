...
class AdminAuthController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        else return view('backend.auth.login', ['page_title' => 'Login']);//MsurJKOO
    }
    ...
    public function login(Request $request)
    {
            ...
            if(auth::check()){
                ...
                if (count($login) > 0)
                {
                    if($this->get_client_ip()==$login[0]['ip_address']){
                        ...
                        return view('backend.auth.login', ['page_title' => 'Login','error'=>"double_attempt"]);
                    }
                }
                $session=new Session;
        ...
        {
            return view('backend.auth.login', ['page_title' => 'Login']);
        }
    }
    ...
    public function forgot(Request $request)
    {
        ...
        if($email==null)return view('backend.auth.login',['page_title' => 'Forgot']);
